<template>
    <div :class="$page.props.showingMobileMenu ? 'block' : 'hidden'"
         class="fixed inset-0 z-20 bg-black opacity-50 transition-opacity lg:hidden"
         @click="$page.props.showingMobileMenu = false"></div>
    ...
    <div :class="$page.props.showingMobileMenu ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'"
        ...
        <div class="flex justify-center items-center mt-8">
            <div class="flex items-center">
                <svg class="w-12 h-12" fill="none" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M364.61 390.213C304.625 450.196 207.37 450.196 147.386 390.213C117.394 360.22 102.398 320.911 102.398 281.6C102.398 242.291 117.394 202.981 147.386 172.989C147.386 230.4 153.6 281.6 230.4 307.2C230.4 256 256 102.4 294.4 76.7999C320 128 334.618 142.997 364.608 172.989C394.601 202.981 409.597 242.291 409.597 281.6C409.597 320.911 394.601 360.22 364.61 390.213Z"
                        fill="#4C51BF" stroke="#4C51BF" stroke-linecap="round" stroke-linejoin="round"
                        stroke-width="2" />
                    <path
        ...
        <nav class="mt-10" x-data="{ isMultiLevelMenuOpen: false }">
            <nav-link :active="route().current('dashboard')" :href="route('dashboard')">
                <template #icon>
            ...
            <nav-link :active="route().current('users.index')" :href="route('users.index')">
                <template #icon>
            ...
            <nav-link :active="route().current('about')" :href="route('about')">
                <template #icon>
                    <svg aria-hidden="true" class="w-5 h-5" fill="none" stroke="currentColor" stroke-linecap="round"
            ...
            <nav-link :active="route().current('blogs.index')" :href="route('blogs.index')">
                <template #icon>
                    <svg aria-hidden="true" class="w-5 h-5" fill="none" stroke="currentColor" stroke-linecap="round"
                         stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24">
                        <path
            ...
            <transition
                ...
                <div v-show="showingTwoLevelMenu">
                    ...
                    <ul aria-label="submenu"
                        class="overflow-hidden p-2 mx-4 mt-2 space-y-2 text-sm font-medium text-white bg-gray-700 bg-opacity-50 rounded-md shadow-inner">
                        <li class="px-2 py-1 transition-colors duration-150">
                            <Link :href="route('rick.morty')" class="w-full">Rick & Morty</Link>
                        </li>
                    </ul>
                    ...
                    <ul aria-label="submenu"
                        class="overflow-hidden p-2 mx-4 mt-2 space-y-2 text-sm font-medium text-white bg-gray-700 bg-opacity-50 rounded-md shadow-inner">
                        <li class="px-2 py-1 transition-colors duration-150">
                            <Link :href="route('advice')" class="w-full">Advice</Link>
                        </li>
                    </ul>
                </div>
            </transition>
